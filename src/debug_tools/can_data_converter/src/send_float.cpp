...
public:
  SendFloatNode() : Node("send_float_node") {
    // declare as int64_t to avoid ambiguity in rclcpp::ParameterValue constructors
...
private:
  void float_callback(const std_msgs::msg::Float32 &msg) {
    ...
    // Copy float bytes into 4-byte array
    ...
    // zero the rest
    for (size_t i = 4; i < 8; ++i) frame_msg.data[i] = 0;
