...
public:
    CANDataConverterNode(const rclcpp::NodeOptions &options = rclcpp::NodeOptions()) 
        ...
        sub_ = this->create_subscription<can_msgs::msg::Frame>(
            "can_rx", 10, 
            [this](const can_msgs::msg::Frame &msg) { this->process_can_frame(msg); });
        
        RCLCPP_INFO(this->get_logger(), 
                   "CAN Data Converter started - filtering ID: 0x%X, publishing to: %s", 
...
private:
    void process_can_frame(const can_msgs::msg::Frame &msg) {
        ...
        if (msg.length == 0 || msg.length > 8) {
            RCLCPP_WARN(this->get_logger(), 
                       "Invalid data length: %d for CAN ID 0x%X", msg.length, msg.id);
            return;
        }
        ...
        RCLCPP_DEBUG(this->get_logger(), 
                    "CAN ID 0x%X: converted %d bytes to int64: %ld", 
                    msg.id, msg.length, converted_value);
    }
    ...
    int64_t convert_to_int64(const std::array<uint8_t, 8> &data, uint8_t length) {
        ...
        // 符号拡張の処理（データ長に応じて）
        if (length < 8) {
            int shift_bits = (8 - length) * 8;
            // 符号ビットが1の場合は符号拡張
            if (data[little_endian_ ? length - 1 : 0] & 0x80) {
