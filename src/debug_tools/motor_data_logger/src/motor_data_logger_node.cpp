...
public:
    MotorDataLogger() : Node("motor_data_logger") {
        ...
        motor_cmd_sub_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
            "motor_commands", 10, std::bind(&MotorDataLogger::motorCommandCallback, this, std::placeholders::_1));

        can_rx_sub_ = this->create_subscription<can_msgs::msg::Frame>(
            "can_rx", 100, std::bind(&MotorDataLogger::canRxCallback, this, std::placeholders::_1));

        timer_ =
            this->create_wall_timer(std::chrono::milliseconds(10), std::bind(&MotorDataLogger::timerCallback, this));
        ...
        voltage_left_ = 0.0f;
        voltage_right_ = 0.0f;
        has_speed_data_ = false;
        has_voltage_data_ = false;
        ...
        RCLCPP_INFO(this->get_logger(),
                    ...
                    "  Auto start: %s",
                    output_file_.c_str(), speed_can_id_, voltage_can_id_, auto_start_ ? "true" : "false");
...
private:
    ...
    void timerCallback() {
        ...
        if (has_speed_data_ || has_voltage_data_) {
            ...
            csv_file_ << std::fixed << std::setprecision(6) << elapsed << "," << target_left_ << "," << target_right_
                      << "," << actual_left_ << "," << actual_right_ << "," << voltage_left_ << "," << voltage_right_
                      << "\n";
    ...
    float actual_right_;
    float voltage_left_;
    float voltage_right_;
    bool has_speed_data_;
    bool has_voltage_data_;
};
