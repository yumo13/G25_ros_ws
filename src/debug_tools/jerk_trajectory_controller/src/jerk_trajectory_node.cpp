...
/**
 * @brief ジャーク制御を用いた速度情報生成ノード
 *
 * 指定した距離、最大速度、最大加速度、最大ジャークの制限を満たしながら
 * y方向への移動を行うcmd_velメッセージを生成する。
 */
...
public:
    JerkTrajectoryController() : Node("jerk_trajectory_controller") {
        ...
        start_sub_ = this->create_subscription<std_msgs::msg::Bool>(
            "start_trajectory", 10, std::bind(&JerkTrajectoryController::startCallback, this, std::placeholders::_1));

        // 制御ループタイマー
        dt_ = 1.0 / control_frequency_;
        control_timer_ = this->create_wall_timer(std::chrono::duration<double>(dt_),
        ...
        RCLCPP_INFO(this->get_logger(),
                    ...
                    "  総時間: %.3f s\n"
                    "  制御周波数: %.1f Hz",
                    target_distance_, max_velocity_, max_acceleration_, max_jerk_, total_time_, control_frequency_);
    }
    ...
    void controlLoop() {
        ...
        if (trajectory_time_ >= total_time_) {
            ...
            if (!trajectory_completed_) {
                ...
                RCLCPP_INFO(this->get_logger(), "軌道完了！ 最終位置: %.6f m (目標: %.6f m), 誤差: %.6f m",
                            current_position_, target_distance_, std::abs(current_position_ - target_distance_));
        ...
        if (static_cast<int>(trajectory_time_ * 10) % 10 == 0) {
            RCLCPP_INFO(this->get_logger(), "時刻: %.2f s, 位置: %.6f m, 速度: %.6f m/s, 加速度: %.6f m/s^2",
